...
class DashboardProvider extends ChangeNotifier {
  ...
  void _calculateOrderStats() {
    ...
    _dashboardStats['totalSales'] = _orders.fold<double>(0, (sum, order) => sum + (order.total ?? 0));
    
    // Calculate average sales per day
    if (_orders.isNotEmpty) {
      final now = DateTime.now();
      final firstOrderDate = _orders.map((o) => o.createdAt).where((date) => date != null).reduce((a, b) => a!.isBefore(b!) ? a : b);
      if (firstOrderDate != null) {
        final daysDiff = now.difference(firstOrderDate).inDays + 1;
        _dashboardStats['avgSalesPerDay'] = _dashboardStats['totalSales'] / daysDiff;
      }
    }
    
    // Count orders by status
    _dashboardStats['pendingOrders'] = _orders.where((o) => o.status == 'pending').length;
    _dashboardStats['processingOrders'] = _orders.where((o) => o.status == 'processing').length;
    _dashboardStats['deliveredOrders'] = _orders.where((o) => o.status == 'delivered').length;
    _dashboardStats['canceledOrders'] = _orders.where((o) => o.status == 'canceled').length;
    _dashboardStats['outForDeliveryOrders'] = _orders.where((o) => o.status == 'out_for_delivery').length;
    _dashboardStats['returnedOrders'] = _orders.where((o) => o.status == 'returned').length;
    _dashboardStats['rejectedOrders'] = _orders.where((o) => o.status == 'rejected').length;
    
    // Get unique customers
    final uniqueCustomers = _orders.map((o) => o.userId).where((id) => id != null).toSet();
  ...
  void _calculateItemStats() {
    ...
    _dashboardStats['featuredItems'] = _items.where((item) => item.isFeatured == true).take(5).toList();
      ...
      if (order.items != null) {
        for (var orderItem in order.items!) {
          if (orderItem.item?.id != null) {
            itemOrderCount[orderItem.item!.id!] = (itemOrderCount[orderItem.item!.id!] ?? 0) + orderItem.quantity;
          }
        }
      }
    }
    
    var sortedItems = _items.where((item) => itemOrderCount.containsKey(item.id)).toList();
    sortedItems.sort((a, b) => (itemOrderCount[b.id] ?? 0).compareTo(itemOrderCount[a.id] ?? 0));
  ...
  void _generateSalesData() {
    ...
    for (int i = 30; i >= 0; i--) {
      ...
      final dailySales = dayOrders.fold<double>(0, (sum, order) => sum + (order.total ?? 0));
  ...
  void _generateCustomerData() {
    ...
    for (int hour = 0; hour < 24; hour++) {
      final hourOrders = _orders.where((order) => 
        order.createdAt != null && 
        order.createdAt!.hour == hour
      ).length;
      
      _customerData.add(CustomerData('${hour.toString().padLeft(2, '0')}:00', hourOrders.toDouble()));
    }
  }
  ...
  void _generateOrderSummaryData() {
    ...
    _orderSummaryData.add(OrderSummaryData('Delivered', (delivered / totalOrders * 100)));
    _orderSummaryData.add(OrderSummaryData('Returned', (returned / totalOrders * 100)));
    _orderSummaryData.add(OrderSummaryData('Canceled', (canceled / totalOrders * 100)));
    _orderSummaryData.add(OrderSummaryData('Rejected', (rejected / totalOrders * 100)));
  }
